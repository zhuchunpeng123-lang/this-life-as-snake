...
## 2026-07-25h · fix(ui): 评级/再来一局恢复原始紧凑居中尺寸(回退 20260725g 拉宽)（build 20260725h）
- **目标**：用户反馈 20260725g 把评级徽标与再来一局按钮拉成满宽太丑,恢复二者原始紧凑居中尺寸;九项卡片维持已修好的满宽对齐(`box-sizing:border-box` 消溢出)。
- **改动文件**：`12_ui.js`（`renderScoreboard` 的 `rbadge` / `btn` 样式回退）。
- **修复**：`rbadge` 还原 `margin:2px auto 14px;display:inline-flex`(内容宽、居中、不拉满);`btn` 还原 `display:block;margin:0 auto 16px`(内容宽、居中、不拉满)。九项卡片保持 `box-sizing:border-box;width:100%` 满宽(=stage/eulogy 同宽)+ 内部图标贴左/标签满宽居中/数值贴右,不溢出、不横拖。
- **取舍**：评级/按钮与九项卡片左右框不再"严格等宽",但用户明确要二者原尺寸比例(紧凑居中);卡片满宽对齐的是「随身结语」框,视觉层级合理(标题/主操作紧凑、明细卡满宽)。
- **是否动 §9 / core / collision / 玩法数值 / Bus 链路 / 阶段1-3 渲染**：**否**（纯 CSS 样式回退）。
- **验收**：①评级 C 与再来一局为原始紧凑居中尺寸(非满宽);②九项卡片满宽对齐、中列居中、数值不溢出不横拖;③console 无溢出告警。

## 2026-07-25g · fix(ui): 结算九项/评级/按钮同左右框 + box-sizing 消右侧溢出（build 20260725g）
- **目标**：封版前最终对齐问题——①九项卡片右侧数值"超框"，内嵌网页需往右拖动才看见；②九项卡片满宽(560px stage)但上方评级徽标/再来一局按钮是 `margin:auto` 内容宽(窄、居中)，左右框不一致。
- **根因**：卡片 `width:100%` 默认 `content-box` + `padding:14px` + 边框 → 实际宽超 stage 出框 → 触发水平滚动条(需往右拖)；且徽标/按钮非满框故与卡片左右边界不齐。
- **改动文件**：`12_ui.js`（`renderScoreboard`）。
- **修复**：①卡片加 `box-sizing:border-box` → `width:100%` 含 padding/边框,精确贴合 stage 内宽,右边缘不再溢出、无需横拖;②评级徽标改 `width:100%;display:flex;justify-content:center`(满框居中)、再来一局按钮改 `width:100%`(满框)→ 与九项卡片**同一左右边界**,三者框对齐;九项中列(标签)整卡满宽 `text-align:center` 与按钮同矢量居中、逐行对齐不偏;图标贴左平行、数值贴右靠右(仍 `max-width:168px` 超宽换行禁截断)。
- **是否动 §9 / core / collision / 玩法数值 / Bus 链路 / 阶段1-3 渲染**：**否**（纯 CSS 布局）。
- **验收**：①评级→按钮→九项三者左右框完全对齐;②九项右数值不溢出、内嵌网页无需横拖即见;③中列「此生长度/走过的路…」整屏居中、与按钮同矢量;④lint 全清、无水平滚动条。

## 2026-07-25f · fix(ui): 结算中列整卡满宽居中(与再来一局按钮同矢量)（build 20260725f）
- **目标**：封版前最后一处——"此生长度/走过的路/斩获"中列未与「再来一条蛇生」按钮同矢量居中。Grid `1fr` 把标签居中在"剩余空间"内,被右侧固定数值列挤偏左,未真正居中。
- **改动文件**：`12_ui.js`（`renderScoreboard` 卡片循环）。
- **修复**：图标/数值改用 `position:absolute` 贴左右边(逐行平行/靠右、垂直居中),标签改为 `display:block;text-align:center;padding:0 44px` 占**整卡满宽**居中→与「再来一局」按钮(`margin:0 auto`)同矢量居中;数值 `max-width:168px` 超宽自动换行仍禁截断。整列逐行居中对齐、且与按钮视觉一致。
- **是否动 §9 / core / collision / 玩法数值 / Bus 链路 / 阶段1-3 渲染**：**否**（纯 CSS 布局）。
- **验收**：①九项中列"此生长度/走过的路/斩获…"在整屏居中、与再来一局按钮同矢量;②图标贴左平行、数值贴右靠右、不截断;③逐行对齐不偏;④lint 全清。

## 2026-07-25e · fix(ui): Combo 未激活文案「部件 未激活」+ 结算九项与结语同宽 + 再来一局上提评级下/九项沉底（build 20260725e）
- **目标**：用户封版前三项反馈——①Combo 未激活态文案过长("未激活·蒸汽爆炸")应改短且与激活态长度一致；②结算九项被压缩(fit-content)比随身结语窄,应与结语同宽、中列仍等列居中;③「再来一局」沉在最底且晚出现,16" 全屏要下滚才见,须上提到评级正下方、与评级同现、九项沉底。
- **改动文件**：`12_ui.js`（`renderComboBadges` / `renderScoreboard` / `startSequence` `after` 时序）。
- **①Combo 未激活文案**：`renderComboBadges` 后缀由 `active?name:('未激活·'+name)` 改 `active?name:'未激活'`→未激活显「🔥+❄️ 未激活」(短,与激活「🔥+❄️ 蒸汽爆炸」长度一致);图标仍置灰(`glyphCol` 用 `STYLE.ui` 灰)、仅双部件齐全(`comboReady`)才高亮 Combo 色+发光。
- **②结算九项与结语同宽 + 中列等列居中**：box 由 `width:fit-content` 改 `width:100%`(=stage 宽,与随身结语 `euWrap` 同宽,视觉一致,不再压缩);每行卡片改 CSS Grid `grid-template-columns:26px 1fr 168px`——图标列 26px 左对齐平行、标签列 `1fr` 居中(因数值列固定 168px 故逐行标签居中位置完全一致=等列居中)、数值列 168px 靠右+正常亮度+超宽自动换行禁截断。满足"左列图标平行 / 中列等列居中 / 右列靠右"。
- **③再来一局上提 + 与评级同现 + 自动滚到按钮**：`startSequence` 删除独立的按钮 `after` 块(原在结语后才出现、沉底);按钮创建移入 `renderScoreboard`,顺序固定为「评级金标 → 再来一局按钮 → 九项卡」(按钮紧贴评级下方、与评级同一次调用出现、零额外延迟);`renderScoreboard` 末尾 `stage.scrollTo({top:rbadge.offsetTop-10,smooth})` 自动滚到评级/按钮,绕过较长结语与九项,16" 全屏无需下滚即见按钮;九项沉到最底(细节可下滚看)。
- **是否动 §9 / core / collision / 玩法数值 / Bus 链路 / 阶段1-3 渲染**：**否**（纯 CSS 布局 + 文案 + 状态着色 + 按钮时序）。
- **验收**：①未激活 Combo 显「部件 未激活」(短)、激活显「部件 名」;②九项整块与结语同宽、中列等列居中、数值靠右不截断(6 位数/长值不破版);③评级→按钮→九项顺序,按钮与评级同现、无需下滚即见;④lint 全清、无悬挂引用、无重复按钮。

## 2026-07-25d · fix(ui): 结算九项三列平行+等列居中 + Combo 徽标未激活置灰 + HUD 还原图标（build 20260725d）
- **目标**：用户实测三项反馈——①结算九项「中列」按各自行自身宽居中导致"有的偏左有的偏右"，并非真·整列居中；②右上 Combo 徽标仅持有一个部件时也用高亮色，未体现"未激活"；③左上 HUD 四指标图标(🐍💀⭐🔥)上一轮被误删。
- **改动文件**：`12_ui.js`（`refreshHUD` `hudData` / `renderComboBadges` / `renderScoreboard`）。
- **①结算九项三列固定宽+整块居中**：box 改 `width:fit-content;margin:auto`(整块在 UI 居中展示)；每张卡片三列均 `flex:none` 固定宽——图标列 `22px`(平行对齐)、标签列 `96px;text-align:center`(整列等列居中,不再各按自身宽居中)、数值列 `150px;text-align:right;white-space:normal`(靠右+正常亮度+超宽自动换行禁截断)。彻底消除中列偏左/偏右；左列图标平行、右列数值靠右对齐。
- **②Combo 徽标未激活置灰**：`renderComboBadges` 原未激活态图标仍用 Combo 高亮色 `col`；现 `glyphCol/nameCol` 在 `!active`(仅持有一部件,comboReady 未满足)时改 `STYLE.ui` 灰度(0.4/0.5)、字面前缀「未激活·X」、边框/底更淡无发光；仅两部件皆持有(`comboReady`)才 `active` 高亮(Combo 色+发光)。例：单吃飞镖→灰显「未激活·电磁炮台/灼烧弹幕」，补齐配对技能→高亮激活。
- **③HUD 还原图标**：`hudData` 四指标还原 `🐍 长度 / 💀 击杀 / ⭐ 得分 / 🔥 连杀 ×`；保留 nowrap 成块 + flex-wrap 超宽换行(绝不裁字,满足 HUD 硬约束)。
- **是否动 §9 / core / collision / 玩法数值 / Bus 链路 / 阶段1-3 渲染**：**否**（纯 CSS 布局 + 文案 + 状态着色）。
- **验收**：①左上有 🐍💀⭐🔥 图标且数值完整(6 位数不破版不截断)；②吃单技能时 Combo 徽标灰显「未激活·X」、齐了才高亮发光；③结算三列平行、中列等列居中、数值靠右不截断；④lint 全清、无悬挂引用。
