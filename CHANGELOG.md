...
- **修复**：`14_main.js` 的 `startIfMenu()` 在 `core.resetRun()` 前同步调用 `Registry.get('audio').unlock()`；保留现有多事件兜底监听，不重写音频节点链。
- **范围**：仅首次用户手势音频解锁时机；未改 BGM/SFX 配方、玩法、数值、伤害、技能、敌人、波次、经济、`03_core.js` 或 `04_collision.js`。
- **验收**：静态检查通过；iOS Safari 普通页面和 standalone 需实测“点击开始后立即有 BGM/音效”。

## 2026-07-28 · fix(input): 摇杆几何缓存降低移动端输入延迟
- **根因**：原 `14_main.js` 在每次 `pointermove` 重复读取画布矩形、安全区和摇杆几何，并重复写入底座/推钮尺寸与位置；这会增加移动端主线程布局读取与样式更新压力。
- **修复**：缓存画布矩形、逻辑缩放、固定锚点、安全区约束、半径和视觉行程；仅在初始化、窗口/旋屏、`ResizeObserver`/`visualViewport` 尺寸变化或 GM 锚点变化时重算。拖动路径只更新手指坐标、方向输入和推钮 `transform`。
- **范围**：仅 `14_main.js` 输入链路与 `index.html` 缓存戳；保留死区 12、基础转向 300°/s、长蛇衰减和边缘撞墙回正规则；未改 gameplay、数值、伤害、技能、敌人、波次、经济、`03_core.js` 或 `04_collision.js`。
- **验收**：静态检查通过；移动端短蛇拖动、旋屏、贴墙和连续变向需按测试清单进行浏览器/真机复验。
...
- **改动（文件清单）**：
  - `02_config.STAGE.segments` 五段时序整体前移 30s：段① 0-30 / 段② 30-150 / 段③ 150-330 / 段④ 330-450 / 段⑤ 450-570（cap/spawnRate/pool 不变）。
  - `index.html` 缓存戳由本仓 pre-commit hook 按 `snake55/*.js` 内容哈希自动 bump（无需手改；hook 已验证生效）。
- **波及/自查（用户指定的前期保底对齐）**：①`rookieProtect` 两段 [0-10,0-30] 与 `lethalProtectSec=30` 均落在新保护期 0-30 内，语义一致；②敌人连杀保底 `GS.stageId !== 1`（09_wave:87）随段①于 30s 结束自动恢复，无"按老 60s 跑"错位；③全仓 grep 无其它硬编码 60 保护期。
- **是否动 §9**：是，STAGE 段时序属 §8 节奏值；**AI 不碰真源 MD，§9 回写由用户完成**。
- **验收 ✅❌**：①保护期 30s 结束即进成长期（wanderer 出现）；②段②起 wanderer+chaser 如期生成；③整局总时长 570s；④教学感（rookieProtect 降速 / 无敌帧）仍在 0-30s 内生效、不丢。
- **未动底层**：03_core/04_collision/伤害管线/coreHp=3/initSegments=3·maxSegments=25/食物=记忆同资源 均未动；段 cap（`segCapByStage`）由 S2 处理，本 commit 不动。
...
## 2026-07-25d~g · fix(ui): 结算/HUD/Combo 封版前迭代背景（build 20260725d→g，落点 20260725h）
- **背景**：封版前围绕结算页与 HUD 的一轮反复打磨（d→g），最终落点见下条 **20260725h**。过程覆盖：①HUD 四指标图标还原(🐍💀⭐🔥)；②Combo 徽标未激活置灰、文案由「未激活·X」收敛为短「未激活」(激活显「部件 名」)；③结算九项与随身结语同宽、中列真正居中（试过 fit-content→grid→absolute 贴边→`box-sizing:border-box` 多方案纠偏，期间解决右侧数值超框需横拖的 bug）；④再来一局上提到评级正下方、与评级同现、自动滚到按钮免 16" 全屏下滚；⑤评级/按钮框对齐来回（g 拉满宽被否，回退 h 恢复紧凑居中）。
- **改动文件**：`12_ui.js`（`refreshHUD` / `renderComboBadges` / `renderScoreboard` / `startSequence` 时序）。
- **是否动 §9 / core / collision / 玩法数值 / Bus 链路 / 阶段1-3 渲染**：**否**（纯 CSS 布局 + 文案 + 状态着色 + 按钮时序）。
- **最终验收落点**：以 **2026-07-25h** 为准（评级/再来一局紧凑居中 + 九项满宽对齐不溢出、中列居中）。
