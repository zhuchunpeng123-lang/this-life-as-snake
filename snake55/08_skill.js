...
var icePending = []                 // ⑥ 首测：延迟发出的 fx:ice_pool（等冰锥飞到落点再出霜环）；GS.timeSec 驱动，pause-safe
	...
	// 局部 AOE：走 collision.queryCircle（半径有界）；过滤 bossBullet；结果复用 _aoeScratch（每帧多次调用，零分配）
	function enemiesIn(x, y, r) {
		var col = Registry.get('collision'); if (!col) { return _aoeScratch }
		var raw = col.queryCircle(x, y, r)
		_aoeScratch.length = 0
		for (var i = 0; i < raw.length; i++) { var e = raw[i]; if (e.active && e.type !== 'bossBullet') { _aoeScratch.push(e) } }
		return _aoeScratch
	}
	...
	for (var j = 0; j < icePools.length; j++) {
		...
		var es = enemiesIn(pp.x, pp.y, effR)             // 🟡 perf 债：每帧每冰池一次 queryCircle 扫描（已登 DEBT）
	...
	function tickBolt(dt) {
		...
		var h = headPos(), es = _enemySnap, maxR2 = SK.bolt.maxRange[i] * SK.bolt.maxRange[i]   // P1-1 射程门控
