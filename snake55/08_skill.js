;(function (global) {
	...
	var timer = { bolt: 0, lightning: 0, steam: 0 }
	var foundCombo = {}
	...
	function tickLightning(dt) {
		...
		var h = headPos(), px = h.x, py = h.y, hops = SK.lightning.chains[i], maxR2 = SK.lightning.maxRange[i] * SK.lightning.maxRange[i]   // P1-1 首跳射程门控
		...
		for (var c = 0; c < hops; c++) {
			...
			if (!best) { break }
			hit[best.id] = true; hurt(best, SK.lightning.damage[i])
	...
	Bus.on('core:run_reset', function () { foundCombo = {}; timer.bolt = 0; timer.lightning = 0; timer.steam = 0 })
