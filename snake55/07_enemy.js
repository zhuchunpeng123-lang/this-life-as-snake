;(function (global) {
	...
	function spawnBullet(x, y, ang) {
		...
		e.lifeT = BOSS_BULLET_LIFE_SEC; e.contact = false; e.slowT = 0; e.slowPct = 0
		list.push(e)
	}
	...
	// 外部（技能）受击入口。isDot=持续伤害（火光环/护盾接触）：不逐帧飘字、不逐帧击退，累计到可读整数再飘（⑥⑦）
	function applyDamage(e, amount, isCrit, isDot) {
		...
		if (!e.kbImmune && !isDot) {                                  // ⑥ 击退：远离蛇头 enemyKnockbackPx（精英/Boss 免疫；DOT 不击退防抖飞）
	...
	var Enemy = {
		list: list, spawn: spawn, applyDamage: applyDamage, applySlow: applySlow,
