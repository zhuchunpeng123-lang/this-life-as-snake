...
var isTouch = false   // 触屏设备标记：init 内赋值；移动端走重排布局、桌面保持原右上三联（供 applyUiScale/按钮文案判断）
	...
	function ensureHudStyle() {
		...
		style.textContent = ''
			...
			+ '.ui-v1-life{width:min(24vw,290px);aspect-ratio:var(--ui-life-ratio);display:flex;align-items:center;justify-content:center;gap:8px;padding:var(--skin-top) var(--skin-right) var(--skin-bottom) var(--skin-left);background-color:transparent;font-size:var(--ui-value);letter-spacing:1px;white-space:nowrap}'
			+ '.ui-v1-life-count{margin-left:4px;font-size:var(--ui-value);font-weight:800;color:var(--ui-text)}.ui-v1-life-hearts{font-size:clamp(19px,2.3vw,27px);letter-spacing:1px;line-height:1}'
	...
	function applyUiTuning() {
		...
		uiVar('--ui-life-stats-gap', getUiTuning('layout.lifeStatsGapPx'), 'px')
