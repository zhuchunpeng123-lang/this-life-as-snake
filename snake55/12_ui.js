...
function capsuleEl(extra) {   // 胶囊芯片(§8.4)：chipBg=panel+panelAlpha 派生，chipBorder=ui 1px，字=textMain；统一柔发光(§四 P1-8)
	...
	function renderScoreboard(stage, cause, win) {
		...
		var btn = mk('button', 'display:block;margin:0 auto 16px;padding:13px 30px;border:2px solid ' + STYLE.player + ';border-radius:12px;background:' + STYLE.player + ';color:' + STYLE.bg + ';font:800 17px system-ui;cursor:pointer;box-shadow:0 0 14px ' + hexA(STYLE.player, 0.5), stage)   // P1-6：主操作=STYLE.player 绿填充+深字+发光
		...
		for (var i = 0; i < rows.length; i++) {
			var card = mk('div', 'display:grid;grid-template-columns:26px 1fr 168px;align-items:center;column-gap:12px;width:100%;padding:7px 14px;border-radius:10px;background:' + hexA(STYLE.panel, 0.35) + ';border:1px solid ' + hexA(STYLE.ui, 0.18) + ';border-left:3px solid ' + STYLE.ui, box)
			mk('span', 'text-align:center;font-size:18px;line-height:1', card).textContent = rows[i][0]   // 图标列：左对齐平行
			var lab = mk('span', 'text-align:center;color:' + STYLE.textMain + ';font:600 14px system-ui', card)   // 标签列：1fr 居中→整列等列居中(数值列固定宽168故逐行对齐)
			lab.textContent = rows[i][1]
			var val = mk('span', 'text-align:right;color:' + STYLE.textMain + ';font-weight:800;font:600 14px system-ui;white-space:normal', card)   // 数值列：固定宽168 靠右、正常亮度、超宽自动换行(禁截断)
