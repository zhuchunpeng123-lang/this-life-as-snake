...
function capsuleEl(extra) {   // 胶囊芯片(§8.4)：chipBg=panel+panelAlpha 派生，chipBorder=ui 1px，字=textMain；统一柔发光(§四 P1-8)
	...
	function renderScoreboard(stage, cause, win) {
		...
		var rbadge = mk('div', 'margin:2px 0 14px;width:100%;display:flex;align-items:center;justify-content:center;gap:8px;padding:6px 18px;border-radius:999px;background:' + hexA(STYLE.food, 0.16) + ';border:1px solid ' + STYLE.food + ';color:' + STYLE.food + ';font:800 15px system-ui;box-shadow:0 0 12px ' + hexA(STYLE.food, 0.4), stage)   // 满框宽(与按钮/九项同左右边界对齐)
		rbadge.innerHTML = '评级<span style="font-size:22px;margin-left:6px">' + computeRating() + '</span>'
		// 再来一局按钮：紧跟评级下方(与评级同时出现,不延迟、不沉底)；九项置于其下最底
		var btn = mk('button', 'display:block;width:100%;margin:0 0 16px;padding:13px 30px;border:2px solid ' + STYLE.player + ';border-radius:12px;background:' + STYLE.player + ';color:' + STYLE.bg + ';font:800 17px system-ui;cursor:pointer;box-shadow:0 0 14px ' + hexA(STYLE.player, 0.5), stage)   // 满框宽(与评级/九项同左右边界对齐)
