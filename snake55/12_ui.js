...
function capsuleEl(extra) {   // 胶囊芯片(§8.4)：chipBg=panel+panelAlpha 派生，chipBorder=ui 1px，字=textMain；统一柔发光(§四 P1-8)
	...
	function renderScoreboard(stage, cause, win) {
		...
		for (var i = 0; i < rows.length; i++) {
			var card = mk('div', 'position:relative;width:100%;min-height:34px;padding:7px 14px;border-radius:10px;background:' + hexA(STYLE.panel, 0.35) + ';border:1px solid ' + hexA(STYLE.ui, 0.18) + ';border-left:3px solid ' + STYLE.ui, box)
