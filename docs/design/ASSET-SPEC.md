...
世界技能实体的交付与接入必须记录以下信息：

- Alpha BBox、主体占比与透明边界；
- optical center、anchor/pivot 及其相对实体坐标；
- 统一的伪俯视角度，避免同一战场内产生透视冲突；
- Lv1 与 Lv5 的实际显示尺寸，而不是只记录源 PNG 画布尺寸；
- PNG 负责实体本身的完整造型，程序 VFX 负责目标方向束线、命中反馈、粒子、爆环和动态状态；
- 必须在真实战场中预览，验证缩放、遮挡、跟随、命中与移动目标可读性；
- PNG 不得烘焙目标方向束线、命中效果或大范围粒子效果，避免与程序层重复叠加。
